// ShelfCheck cover-art layer. Identity covers + verified retail fronts.
(()=>{
  const titleCovers={
  ...
  const restoreDossier=()=>{if(!reopenDossier)return;reopenDossier=false;const dlg=dossierDialog();if(dlg&&!dlg.open)dlg.showModal()};
  const closeLightbox=()=>{if(lightbox.open)lightbox.close();else{clearLightbox();restoreDossier()}};
  const openLightbox=(url,title)=>{if(!url)return;const img=lightbox.querySelector('img');img.src=url;img.alt=(title||'Game')+' cover';const dlg=dossierDialog();reopenDossier=!!(dlg&&dlg.open);if(reopenDossier)dlg.close();requestAnimationFrame(()=>{if(!lightbox.open)lightbox.showModal()})};
  lightbox.addEventListener('click',e=>{if(e.target===lightbox||e.target.tagName==='BUTTON')closeLightbox()});
  lightbox.addEventListener('close',()=>{clearLightbox();requestAnimationFrame(restoreDossier)});

  const paint=()=>{document.querySelectorAll('#results article.card').forEach(card=>{if(card.querySelector('.cover-shell'))return;const title=card.querySelector('.top b')?.textContent||'';const badge=card.querySelector('.badge')?.textContent||'';const isProduct=/COLLECTION|BEST WAY TO BUY/.test(badge);const p=isProduct&&typeof findProduct==='function'?findProduct(title):null;const x=!isProduct?(items.find(g=>g.title===title)||items.find(g=>norm(g.title)===norm(title))):null;const shell=document.createElement('div');shell.className='cover-shell';const url=p?productCover(p):(x?coverFor(x):null);if(url){const img=document.createElement('img');img.loading='lazy';img.decoding='async';img.alt=(p?.title||x?.title||title)+' cover';img.src=url;img.onerror=()=>{shell.innerHTML=fallbackHtml};shell.appendChild(img)}else shell.innerHTML=fallbackHtml;card.prepend(shell);if((x&&effectiveStatus(x)==='OWNED')||(p&&productSet.has(p.key)))card.classList.add('cover-owned')})};
  ...
  retailScript.onerror=()=>console.warn('ShelfCheck: verified retail cover layer unavailable; using curated/legacy covers.');
