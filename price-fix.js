// ShelfCheck price repair: imported PriceCharting profiles may override the shared public
// snapshot, but baked-in audited CIB values must work on every device without an import.
...
function priceFor(x){
  // A richer browser-local profile wins when present.
  let p=priceMap.get(norm(x.title));if(p)return p;
  for(const a of aliasesById.get(x.id)||[]){p=priceMap.get(a);if(p)return p}
  // Public pricing patches write their audited CIB value to x.max. Treat that as the
