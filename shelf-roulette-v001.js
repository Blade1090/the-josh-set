...
(()=>{
  ...
  function cardHtml(x,i){const cover=window.SHELFCHECK_COVER_ART?.coverFor?.(x);const coverHtml=cover?`<img src="${esc(cover)}" alt="" loading="lazy" decoding="async" onerror="__rouletteCoverError(this)">`:'<div class="cover-fallback">PS4<br>COVER</div>';const hook=hookText(x);return `<article class="roulette-card" data-id="${x.id}" style="animation-delay:${i*90}ms"><div class="roulette-cover" onclick="detail(${x.id})">${coverHtml}</div><div class="roulette-info" onclick="detail(${x.id})"><b>${esc(x.title)}</b><small class="roulette-time">${esc(timeLabel(x))}</small>${hook?`<p class="roulette-hook">${esc(hook)}</p>`:'<p class="roulette-hook muted">No dossier hook yet.</p>'}</div><div class="roulette-actions"><button class="roulette-played" onclick="event.stopPropagation();shelfRouletteMarkPlayed(${x.id})">✓ PLAYED THIS</button><button class="roulette-beaten" onclick="event.stopPropagation();shelfRouletteMarkBeaten(${x.id})">🏆 BEAT THIS</button></div></article>`}
  ...
  shelfRouletteDeal=renderHand;shelfRouletteOpen=openShelfRoulette;shelfRouletteMarkPlayed=markPlayed;shelfRouletteMarkBeaten=markBeaten;
  ...
  window.SHELFCHECK_ROULETTE={version:3,openShelfRoulette,eligiblePool,isShortNight,isPlayed,isBeaten,setStatus};
})();
