...
(()=>{
  ...
  // Every game dealt this Roulette session (any hand, whether later rejected via NOPE or just
  // replaced by DEAL AGAIN) goes in here and is never redealt until the eligible pool is actually
  // exhausted, at which point the bag resets and starts over -- same shuffle-bag/no-repeat
  // philosophy fun-features-v103.js's Random Game already uses, just tracked across whole hands
  // instead of single picks. Reset only on open/reopen (openShelfRoulette); toggling a filter or
  // marking a game played/beaten does not clear it.
  ...
  function dealHand(){const pool=eligiblePool();let candidates=pool.filter(x=>!sessionSeenIds.has(x.id));if(candidates.length<Math.min(3,pool.length)){sessionSeenIds=new Set();candidates=pool}const hand=shuffle(candidates.slice()).slice(0,Math.min(3,candidates.length));hand.forEach(x=>sessionSeenIds.add(x.id));return{hand,poolSize:pool.length}}
  ...
  function pickForMe(){
    ...
    const settle=()=>{
      ...
      // Once a winner is showing, the SAME button becomes the reject-the-whole-hand action --
      // pressing it again must not just re-pick among these same three (that was the bug: you
      // could cycle the identical hand forever). See nopeRejectHand().
      if(btn){btn.textContent='❌ NOPE — PICK AGAIN';btn.disabled=false;btn.onclick=nopeRejectHand}
  ...
  // NOPE -- PICK AGAIN: rejects the entire currently-shown hand (already permanently recorded in
  // sessionSeenIds the moment it was dealt -- dealHand() itself is what enforces "never redeal an
  // already-seen game this session"), deals a fresh eligible hand, and immediately offers a new
  // recommendation on it so repeated presses keep walking forward through new games instead of
  // re-picking among the same stale three.
  function nopeRejectHand(){
    if(picking)return;
    renderHand();
    scrollRouletteTop();
    pickForMe();
  }
  function markPlayed(id){setStatus(id,'PLAYED');sessionSeenIds.delete(id);renderHand()}
  function markBeaten(id){setStatus(id,'BEATEN');sessionSeenIds.delete(id);renderHand()}
  function openShelfRoulette(){sessionSeenIds=new Set();renderHand();dlg.showModal();dlg.scrollTop=0}
  ...
  window.SHELFCHECK_ROULETTE={version:6,openShelfRoulette,eligiblePool,isShortNight,isPlayed,isBeaten,setStatus,roleFor,pickForMe,nopeRejectHand,get currentHandIds(){return currentHand.map(x=>x.id)},get winnerId(){return winnerId},get picking(){return picking},get sessionSeenIds(){return[...sessionSeenIds]}};
})();
