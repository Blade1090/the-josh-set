// ShelfCheck v0.69 — exact, auditable GameEye ownership reconciliation.
// Every satisfied identity is traceable to one GameEye row. Product matching uses the
// same merged-product model as ShelfCheck, and the accounting proves how boxes become identities.
(()=>{
  ...
  importCSV=async function(f){
    ...
    for(const r of rows){
      ...
      for(const c of cs){
        ...
        const before=owned.size;ids.forEach(id=>owned.add(id));
        const addedUnique=owned.size-before;
        ownedProducts.add(p.key);hit=true;
        ledger.push({gameEye:title,matchType:ids.length>1?'MULTI_IDENTITY_PRODUCT':'PRODUCT',matched:p.title,ids,identities:ids.map(id=>byId.get(id)?.title).filter(Boolean),coverageCount:ids.length,grossBonus:Math.max(0,ids.length-1),addedUnique,overlap:ids.length-addedUnique});
        break;
      }
      ...
      if(!hit){
        ...
        if(ex){excluded++;hit=true;ledger.push({gameEye:title,matchType:'EXCLUDED',matched:ex.title,ids:[],identities:[],coverageCount:0,grossBonus:0,addedUnique:0,overlap:0});}
      }
    ...
    const matched=ledger.filter(x=>x.matchType==='IDENTITY'||x.matchType==='PRODUCT'||x.matchType==='MULTI_IDENTITY_PRODUCT');
    const matchedRows=matched.length;
    const grossBonus=multi.reduce((n,x)=>n+x.grossBonus,0);
    const overlap=matched.reduce((n,x)=>n+x.overlap,0);
    const netCompilationGain=owned.size-matchedRows;
    const rowAccountingOK=titles===matchedRows+excluded+unmatched.length;
    const identityAccountingOK=owned.size===matched.reduce((n,x)=>n+x.addedUnique,0);
    const bonusAccountingOK=netCompilationGain===grossBonus-overlap;

    saveState({...stateCache,version:12,owned:[...owned],products:[...ownedProducts],source:f.name,ownershipAudit:{at:new Date().toISOString(),ps4Rows:titles,matchedRows,satisfied:owned.size,excluded,unresolved:unmatched.length,netCompilationGain,rowAccountingOK,identityAccountingOK,bonusAccountingOK}});
    ...
    window.SHELFCHECK_OWNERSHIP_AUDIT=audit;
    const proof=rowAccountingOK&&identityAccountingOK&&bonusAccountingOK?'ACCOUNTING VERIFIED':'AUDIT WARNING';
    const msg=`GameEye: ${titles} PS4 rows → ${owned.size} satisfied · +${netCompilationGain} net compilation identities · ${excluded} excluded · ${unmatched.length} unresolved · ${proof}.`;
    $('#syncmsg').textContent=msg;
    console.group('ShelfCheck GameEye ownership audit');console.log(msg);console.log('Accounting',audit);console.table(multi);if(unmatched.length)console.warn('Unresolved',unmatched);console.log('Full reconciliation ledger',ledger);console.groupEnd();
  };

  window.SHELFCHECK_OWNERSHIP_AUDIT_V069=true;
})();
