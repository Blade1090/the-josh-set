...
//   -> all 55 dossier-overrides-N.js files -> dossier-apply.js (the real override-merge logic)
...
// dossier-overrides.js is batch 1 (no numeric suffix); dossier-overrides-2.js..-55.js are 2-55.
// Kept in sync with dossier-apply.js's own DOSSIER_OVERRIDES/_2/../_55 global name list by hand.
const OVERRIDE_BATCHES = Array.from({ length: 55 }, (_, i) => {
  const n = i + 1;
...
async function main() {
  ...
  const report = {
    ...
    method: 'Runtime resolution trace: app.js -> model-fix.js -> full census-mutator pipeline -> census-finalize.js -> dossiers.js (dossierFor/dossierQuality/auditDossiers, real) -> all 55 dossier-overrides-N.js -> dossier-apply.js (real override merge). Coverage is never judged by object existence alone.',
