...
//   -> all 54 dossier-overrides-N.js files -> dossier-apply.js (the real override-merge logic)
...
// dossier-overrides.js is batch 1 (no numeric suffix); dossier-overrides-2.js..-54.js are 2-54.
// Kept in sync with dossier-apply.js's own DOSSIER_OVERRIDES/_2/../_54 global name list by hand.
const OVERRIDE_BATCHES = Array.from({ length: 54 }, (_, i) => {
  const n = i + 1;
...
async function main() {
  ...
  const report = {
    ...
    method: 'Runtime resolution trace: app.js -> model-fix.js -> full census-mutator pipeline -> census-finalize.js -> dossiers.js (dossierFor/dossierQuality/auditDossiers, real) -> all 54 dossier-overrides-N.js -> dossier-apply.js (real override merge). Coverage is never judged by object existence alone.',
