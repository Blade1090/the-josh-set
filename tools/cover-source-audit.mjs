...
ctx.censusQueue.add.length=0;ctx.censusQueue.exclude.length=0;
...
let launchbox={},launchboxTitles={},launchboxReconstructed={};
if(fs.existsSync('cover-launchbox-retail.js')){
  const lb=evalFile('cover-launchbox-retail.js');
  launchbox=lb.SHELFCHECK_LAUNCHBOX_COVERS||{};
  launchboxTitles=lb.SHELFCHECK_LAUNCHBOX_TITLES||{};
  launchboxReconstructed=lb.SHELFCHECK_LAUNCHBOX_RECONSTRUCTED_TITLES||{};
}
...
for(const x of items.filter(v=>v.set==='INCLUDED').sort((a,b)=>a.title.localeCompare(b.title))){
  ...
  else if(launchboxReconstructed[key]){source='LAUNCHBOX_RECONSTRUCTED';url=launchboxReconstructed[key]}
...
fs.writeFileSync('audit-out/cover-source-audit.json',JSON.stringify(out,null,2));
